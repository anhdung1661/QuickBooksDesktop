...
/**
 * Class AddCreditMemoToQueue
 * @package Magenest\QuickBooksDesktop\Controller\Adminhtml\Queue
 */
class AddCreditMemoToQueue extends Action
{
    ...
    /**
     * AddCreditMemoToQueue constructor.
    ...
    public function execute()
    {
        ...
        try {
            list($numberAdded, $numberNotAdded) = $this->_queueAction->addCreditMemosToQueue();

            $this->messageManager->addSuccessMessage(
                __(sprintf('Totals %s credit memo(s) have been added to Queue', $numberAdded))
            );
            if ($numberNotAdded) {
                $this->messageManager->addNoticeMessage(
                    __(sprintf('Totals %s  credit memo(s) cannot add to Queue', $numberNotAdded))
                );
            }
